...
/**
 * 封装UUID
 * 随机数工具，可以生成 32 16 64 位的 UUID ，也可以生成任意长度的随机字符串，
 * 或者区分大写小写
...
public abstract class RandomUtils {
    ...
    /**
     * @return 64进制表示的紧凑格式的 RandomUtils
     */
    ...
    /**
     * 返回一个 RandomUtils ，并用 64 进制转换成紧凑形式的字符串，内容为 [\\-0-9a-zA-Z_]
     * <p>
     * 比如一个类似下面的 RandomUtils:
     * 
     * <pre>
     * a6c5c51c-689c-4525-9bcd-c14c1e107c80
     * 一共 128 位，分做L64 和 R64，分为为两个 64位数（两个 long）
     *    > L = uu.getLeastSignificantBits();
     *    > RandomUtils = uu.getMostSignificantBits();
    ...
    /**
     * 从一个 UU64 恢复回一个 RandomUtils 对象
     * 
     * @param uu64
     *            64进制表示的 RandomUtils, 内容为 [\\-0-9a-zA-Z_]
     * @return RandomUtils 对象
     */
    ...
    /**
     * 将紧凑格式的 UU16 字符串变成标准 RandomUtils 格式的字符串
     * 
     * @param uu16
     * @return 标准 RandomUtils 字符串
     */
     ...
     * 
     * @param uu64
     *            uu64 64进制表示的 RandomUtils, 内容为 [\\-0-9a-zA-Z_]
     * @return 16进制表示的紧凑格式的 RandomUtils
     */
     ...
     * @return
     */
    public static String captchaNumberAndChar(int length) {
        ...
        Random rand = new Random();
        for (int i = 0; i < length; i++) {
            sb.append(target[rand.nextInt(target.length - 1)]);
        }
        return sb.toString();
    }
    ...
    public static void main(String[] args){
        System.out.println(captchaChar(10,true));
    }
}